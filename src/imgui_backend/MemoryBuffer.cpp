...
MemoryBuffer::MemoryBuffer(size_t size, void* bufferPtr, nvn::MemoryPoolFlags flags) {
    ...
    bd->memPoolBuilder.SetDefaults().SetDevice(bd->device).SetFlags(flags).SetStorage(memBuffer, size);
...
void MemoryBuffer::ClearBuffer() { memset(memBuffer, 0, pool.GetSize()); }